...
#include "ConstraintLocatorPathElts.def"
  ...
  /// Determine whether this locator points to the wrapped value type mismatch.
  bool isForAllowedWrappedValueMismatch() const;
  ...
  /// If this locator points to wrapped value type mismatch.
  TypeBase *getAllowedWrappedValueMismatch() const;
...
class LocatorPathElt::AllowWrappedValueMismatch final : public StoredPointerElement<TypeBase> {
public:
    AllowWrappedValueMismatch(TypeBase *type)
    : StoredPointerElement(PathElementKind::AllowWrappedValueMismatch, type) {
  }
  ...
  static bool classof(const LocatorPathElt *elt) {
    return elt->getKind() == PathElementKind::AllowWrappedValueMismatch;
  }
};
